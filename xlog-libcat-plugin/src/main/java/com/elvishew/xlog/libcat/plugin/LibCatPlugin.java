...
/**
 * Registers LibCat bytecode instrumentation for Android application variants.
 */
public class LibCatPlugin implements Plugin<Project> {
  ...
  public void apply(Project project) {
    project.getPluginManager().withPlugin("com.android.application", ignored -> {
      ...
      androidComponents.onVariants(androidComponents.selector().all(),
          (Function1<ApplicationVariant, Unit>) variant -> {
            configureInstrumentation(variant);
            return Unit.INSTANCE;
          });
    });
  }

  private void configureInstrumentation(ApplicationVariant variant) {
    variant.getInstrumentation().transformClassesWith(
        LogClassVisitorFactory.class,
